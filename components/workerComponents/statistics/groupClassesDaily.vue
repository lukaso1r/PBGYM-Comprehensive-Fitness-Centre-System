...
onMounted(() => {
  statisticStore.getGroupClassesDaily(); // Pobierz dane dzienne na temat zajęć grupowych
});
...
// Obliczenia ilości zajęć grupowych dla wybranego miesiąca
const groupClassesDaily = computed(() => {
  ...
  return Array.from({ length: daysInMonth }, (_, dayIndex) => {
    ...
    const total = statisticStore.groupClassesDaily[formattedDay] || 0;
...
watch([selectedYear, selectedMonth], () => {
  ...
  statisticStore.getGroupClassesDaily(); // Pobierz dane dla nowego miesiąca i roku
});
</script>

<template>
  <div class="groupClassesDaily col-span-2 blockCustomShadow grid grid-cols-1 rounded-lg p-4 bg-white gap-4">
    <p class="font-semibold text-lg">
      Ilość zajęć grupowych dziennie - <span class="font-normal text-slate-500">StatisticStore.groupClassesDaily</span>
    </p>
    ...
    <!-- Wykres ilości zajęć grupowych dziennych -->
    <div class="total-daily-group-classes col-span-1 grid grid-cols-1 rounded-lg p-4 bg-white gap-4">
      <BarChart
        index="name"
        :data="groupClassesDaily"
        ...
        :x-axis-options="{
          ...
          ticks: {
            autoSkip: false,
            maxTicksLimit: 31,
            callback: (value, index) => groupClassesDaily[index]?.name || '',
          }
        }"
        :colors="['#203983']"
        :rounded-corners="4"

...
onMounted(() => {
  statisticStore.getMemberRegistrationsDaily(); // Pobierz dane dzienne na temat rejestracji członków
});
...
// Obliczenia rejestracji członków dziennie dla wybranego miesiąca
...
watch([selectedYear, selectedMonth], () => {
  ...
  statisticStore.getMemberRegistrationsDaily(); // Pobierz dane dla nowego miesiąca i roku
});
</script>
...
<template>
  <div class="memberRegistrationsDaily col-span-2 blockCustomShadow grid grid-cols-1 rounded-lg p-4 bg-white gap-4">
    ...
    <!-- Wykres rejestracji członków dziennych -->
    <div class="total-daily-registrations col-span-1 grid grid-cols-1 rounded-lg p-4 bg-white gap-4">
      <BarChart
        index="name"
        :data="memberRegistrationsDaily"
        :categories="['Rejestracje członków']"

...
const calendar = computed(() => {
    ...
    days.forEach(day => {
        ...
        if(prop.singleClass){
            ...
            isSameDay(new Date(groupClass.dateStart), day)).concat(prop.groupClassesHistory.filter(groupClass => isSameDay(new Date(groupClass.dateStart), day)));
        }
...
const onClickedClass = async (groupClass: GroupClassWithTrainer) => {
    ...
    await groupClassesStore.getGroupClassesMembers(clickedClasses.value.id);
...
<template>
    ...
    <UCard :ui="{ ring: '', divide: 'divide-y divide-gray-100 dark:divide-gray-800' }">
        ...
        <div class="w-full flex flex-col gap-8">
            ...
            <WorkerComponentsGroupClassesEnroledMembersList :group-class-obj="clickedClasses" :group-class-members="groupClassesStore.groupClassesMembers" :hideShadow="true"/>
        </div>
        <template #footer>
